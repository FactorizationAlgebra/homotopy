...
We first describe the necessity of deriving limits and colimits. Let $I$ be a small category, and consider two diagrams $D,D':I\rightarrow \cat{Top}$. If one has a natural transformation $f:D\rightarrow D'$, then there is an induced map colim$D\rightarrow\text{colim}D'$. If $f$ is a natural weak equivalence ($D(i)\rightarrow D'(i)$ is a weak equivalence for all $i$), it does not in general follow that colim$D\rightarrow\text{colim}D'$ is a weak equivalence. 

%----------------------------------------------------------------------------------------------------------------------------------------------
\section{Fibrations and Cofibrations}

\subsection{Compactly Generated Spaces}
    Given a map $f:X\times Y\rightarrow Z,$ we would like to topologize the set of continuous functions $C(Y,Z)$ in such a way that $f$ is continuous if and only if the adjoint \[\tilde{f}:X\rightarrow C(Y,Z), \tilde{f}(x)(y)=f(x,y)\] is continuous. For instance:
    \begin{enumerate}
    \item We would like an action of a topological group $G\times Z\rightarrow Z$ to correspond to a continuous function $G\rightarrow \text{Hom}_{iso}(Z,Z)$, where $\text{Hom}_{iso}$ is given the subspace topology. 
    \item We would like a homotopy $f:I\times Y\rightarrow Z$ to correspond to a path $\tilde{f}:I\rightarrow C(Y,Z)$ of functions.
    \item The evaluation map \[C(Y,Z)\times Y\rightarrow Z, (f,y)\mapsto f(y)\] should be continuous.
    \end{enumerate}
    
    Such a topology does not exist for $\cat{Top}.$ However, we can restrict to a full subcategory of spaces called "compactly generated" spaces. 
    \begin{defn}
    A topological space $X$ is said to be \textbf{compactly generated} if $X$ is Hausdorff and a subset $A\subset X$ is closed if and only if $A\cap C$ is closed for every compact $C\subset X.$
    \end{defn}
...
\end{tikzcd}
\]
where $LV$ is the last vertex map, mapping $sd(\Delta^{n})=C(sd(\partial\Delta^{n}))\rightarrow\Delta^{n}.$
